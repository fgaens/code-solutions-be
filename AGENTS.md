...
- Demos load only on demand, without a launch control: `assets/js/site.js` creates the `iframe` when an `IntersectionObserver` brings the section within 200px of the viewport, or when a tab is selected. Do not add `src` to the markup; the demo URL, frame title, and loading label live in `data-demo-*` attributes on `.demo-viewport`. The loader is idempotent per viewport, so visited frames stay mounted across tab switches and keep their state. Never focus the frame on load, and keep the tablist's `aria-selected`/`tabindex` handling in that script.
